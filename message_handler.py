...
class Handler:
    ...
    async def handle_message(self, message):
        ...
        if self.client.user.mentioned_in(message):  # Fixed method name here
            ...
            clean_content = message.content.replace(f'<@{self.client.user.id}>', '').strip()
            if clean_content in ['', ' ', None]:
                await self.send_message_to_discord(message.channel.id, 'Please provide a command or message.')
    ...
    async def _handle_command(self, channel_id, command, content):
        ...
        match command:
            case '/commands':
                ...
                await self.send_message_to_discord(channel_id, f'Available commands: \n{n}')
            case '/yt':
                try:
                    url = content.split(' ')[1]
                except IndexError:
                    await self.send_message_to_discord(channel_id, 'Please provide a YouTube URL.')
                    return
                if not url or not url.startswith('https://www.youtube.com/watch'):
                    await self.send_message_to_discord(channel_id, 'Please provide a YouTube URL.')
                    return
                
                await self.send_message_to_discord(channel_id, f'Fetching subtitles for provided URL')
                yt = YouTube(url)
                subtitles = yt.get_subtitles()
                if subtitles:
                    await self.send_message_to_discord(channel_id, f'Subtitles found, trying to summarize')
                    # Send the subtitles to n8n webhook
                    response = self._send_to_n8n_webhook(f'Sumarize the following subtitles:\n{subtitles}')
                    if response.status_code == 200:
                        await self.send_message_to_discord(channel_id, 'Subtitles sent to n8n webhook successfully.')
                    else:
                        await self.send_message_to_discord(channel_id, 'Failed to send subtitles to n8n webhook.')
                else:
                    await self.send_message_to_discord(channel_id, 'No subtitles available for this video.')
